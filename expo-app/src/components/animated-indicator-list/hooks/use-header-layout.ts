...
import { isHeader, type HeaderListItem, type ListItem } from "../constants";
import type { MeasureableAnimatedViewRef } from "../measurable-animated-view";
...
const useHeaderLayout = ({ headers, data, itemHeight, headerHeight }: UseHeaderLayoutParams) => {
  ...
  const headersLayoutX = useDerivedValue<Record<string, LayoutRectangle>>(() => {
    ...
    return headers.reduce((acc, { header }, i) => {
      ...
      if (ref.current && ((acc as any)?.[header] == null || (acc as any)?.[header]?.width === 0)) {
