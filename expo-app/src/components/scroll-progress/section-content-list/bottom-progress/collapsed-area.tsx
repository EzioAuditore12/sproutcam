...
const CollapsedArea: FC<CollapsedAreaProps> = memo(({ state, readingTime, onReset }) => {
  const rCollapsedAreaStyle = useAnimatedStyle(() => {
    const isCollapsed =
      state.get() === BottomProgressState.INITIAL || state.get() === BottomProgressState.END;
    return {
  ...
  return (
    ...
    >
      ...
      <Animated.View style={[StyleSheet.absoluteFill, rUpArrowContainerStyle]}>
...
CollapsedArea.displayName = "CollapsedArea";
