...
export const CircularButton: React.FC<CircularButtonProps> = ({
  ...
  const activeIcon = useMemo(() => {
    ...
    return <FontAwesome6 name="check" size={size / 2} color={lightColor} iconStyle="solid" />;
  }, [activeIconProp, size]);

  const rBaseIconStyle = useAnimatedStyle(() => {
    const rotate = interpolate(progress.get(), [0, 0.5], [0, 45], Extrapolation.CLAMP);
    return {
  ...
  const rActiveIconStyle = useAnimatedStyle(() => {
    const rotate = interpolate(progress.get(), [0, 1], [-45, 0], Extrapolation.CLAMP);
    return {
