...
const ListItem: React.FC<ListItemProps> = ({ index, scrollOffset }) => {
  ...
  const visibleAmount = useDerivedValue(() => {
    ...
    const distanceFromTop = scrollOffset.get() + BottomThreshold - baseTranslateY;
  ...
  const realTranslateY = useDerivedValue(() => {
    return interpolate(
      ...
      [scrollOffset.get() + BottomThreshold - FullListItemHeight, baseTranslateY],
      Extrapolation.CLAMP
    );
  }, []);
  ...
  const scale = useDerivedValue(() => {
    return interpolate(visibleAmount.get(), [0, FullListItemHeight], [0.6, 1], Extrapolation.CLAMP);
  }, [baseTranslateY]);
  ...
  return (
    <View style={styles.container}>
      <Animated.FlatList
        ...
        renderItem={({ index }) => <ListItem index={index} scrollOffset={scrollOffset} />}
